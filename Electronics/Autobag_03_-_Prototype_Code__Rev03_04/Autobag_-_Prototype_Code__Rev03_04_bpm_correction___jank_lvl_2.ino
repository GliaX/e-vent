...
const int LCD_COLS = 16;            // Number of columns on LCD display
const int LCD_ROWS = 2;             // Number of rows on LCD display
...
const float HOMESPEED = 500.0;      // The speed for homing (steps per second)
...
const float maxStrokeFinger = 27.0; // Maximum allowable stroke of finger (degrees)
const float gearR = 16*5;             // Gear Ratio between stepper and finger, default is 16, including the gearbox is an additional *5
const float StepsPerRev = 200;     // Steps per revolution (setting on Applied Motion STR4 driver)
...
void setup() {
    ...
    stepper.setPinsInverted(true,false,false);                 // Direction pin inverted so that compression is positive direction
